...
grouped_by_class = combined_df.groupby('class')

# Visualize the data for each class
for class_name, group in grouped_by_class:
    plt.figure(figsize=(15, 8))
    group[numeric_columns].hist(bins=30, alpha=0.5, layout=(2, 3), edgecolor='black')
    plt.suptitle(f'Histograms of Numeric Features for Class: {class_name}')
    plt.show()

    group.boxplot(column=numeric_columns.tolist(), figsize=(15, 8))
    plt.title(f'Boxplots of Numeric Features for Class: {class_name}')
    plt.show()